...
def load_data(data_set_name, data_path='./dataset'):
...
def data_pad_zero(data, patch_length):
    data_padded = np.pad(data, ((patch_length, patch_length), (patch_length, patch_length), (0, 0)), 'constant',
                         constant_values=0)
    return data_padded.astype(np.float32)  # 转换为 float32
...
def generate_iter_1(data_padded, hsi_h, hsi_w, label_reshape, index, patch_length, batch_size, model_type_flag, model_3D_spa_flag):
    ...
    train_iter = Data.DataLoader(dataset=torch_dataset_train, batch_size=batch_size, shuffle=True, num_workers=8, pin_memory=True)
    val_iter = Data.DataLoader(dataset=torch_dataset_val, batch_size=batch_size, shuffle=True, num_workers=8, pin_memory=True)
    test_iter = Data.DataLoader(dataset=torch_dataset_test, batch_size=batch_size, shuffle=False, num_workers=8, pin_memory=True)
...
def generate_auxilary_iter(data_padded, hsi_h, hsi_w, label_reshape, aux_index, patch_length, batch_size, model_type_flag,
    ...
    if last_batch_flag == 1:
        aux_iter = Data.DataLoader(dataset=torch_dataset_aux, batch_size=batch_size, shuffle=True, num_workers=8,
                                     drop_last=True)
    else:
        aux_iter = Data.DataLoader(dataset=torch_dataset_aux, batch_size=batch_size, shuffle=True, num_workers=8)
...
def generate_iter_2(data_padded, hsi_h, hsi_w, label_reshape, index, patch_length, batch_size, model_type_flag,
    ...
    total_iter = Data.DataLoader(dataset=torch_dataset_total, batch_size=batch_size, shuffle=False, num_workers=8)
...
def generate_all_iter(data, labels, patch_length,batch_size, device, model_type_flag, model_3D_spa_flag,all_index):
    ...
    all_iter = Data.DataLoader(dataset=torch_dataset, batch_size=batch_size, shuffle=False, num_workers=8)
    return all_iter